...
local function enable(win, opts, record_attributes)
	...
	if opts.fullscreen ~= win:get_dimensions().is_full_screen then
		...
		if not opts.fullscreen and opts.maximize then
			win:maximize()
		end
	else
		if opts.fullscreen and presentation_fullscreen then
			...
			if presentation_fullscreen and presentation_maximize then
				win:maximize()
			end
		end
	end
	...
	if opts.presentation.enabled then
		table.insert(config.keys, {
			...
			action = wezterm.action_callback(function(win)
				toggle(win, {
					fullscreen = false,
					maximize = opts.presentation.maximize or opts.maximize,
